...
module Simplex
  class MaximizationProblemBuilder
    ...
    def build_objective_vector(objective_coefficients, number_of_slack_variables)
      coefficients_on_opposite_side_of_equation =
        objective_coefficients.map { |coefficient| -1 * coefficient }

      slack_variable_placeholders = Array.new(number_of_slack_variables, 0)

      coefficients_on_opposite_side_of_equation + slack_variable_placeholders
    end

    def build_constraints_matrix(constraints, number_of_slack_variables, number_of_non_slack_variables)
